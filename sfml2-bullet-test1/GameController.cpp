...
void GameController::OnInit() {
	...
	// create our debug drawing object and pass it to Bullet
	debugDraw = new DebugDraw(window);
	//debugDraw->setDebugMode(btIDebugDraw::DBG_DrawWireframe);
	dynamics_world->setDebugDrawer(debugDraw);
	...
	btDefaultMotionState* fall_motion_state = new btDefaultMotionState(btTransform(btQuaternion(0, 0, 0, 1), btVector3(0, 50, 0)));
	btScalar mass = 1;
	btVector3 fall_inertia(0, 0, 0);
	fall_shape->calculateLocalInertia(mass, fall_inertia);
	btRigidBody::btRigidBodyConstructionInfo fall_rigid_body_ci(mass, fall_motion_state, fall_shape, fall_inertia);
	...
	btScalar box_mass = 1;
	btVector3 box_inertia(0, 0, 0);
	btCollisionShape* box_shape = new btBoxShape(btVector3(1, 1, 1));
	box_shape->calculateLocalInertia(box_mass, box_inertia);
	btDefaultMotionState* box_motion_state = new btDefaultMotionState(btTransform(btQuaternion(0, 0, 0, 1), btVector3(0, 50, 0)));
	btRigidBody::btRigidBodyConstructionInfo box_body_ci(box_mass, box_motion_state, box_shape, box_inertia);
	body = new btRigidBody(box_body_ci);
