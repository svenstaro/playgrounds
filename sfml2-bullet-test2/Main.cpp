...
int main(int argc, char** argv) {
        ...
        boost::shared_ptr<btGhostPairCallback> ghostpaircallback(new btGhostPairCallback);
        ...
        boost::shared_ptr<btGhostObject> ghostObject(new btGhostObject());
        boost::shared_ptr<btSphereShape> ghostShape(new btSphereShape(5));
        ghostObject->setCollisionShape(ghostShape.get());
        ghostObject->setWorldTransform(btTransform(btQuaternion(0,0,0,1),btVector3(1,1,0)));
        ghostObject->setCollisionFlags(ghostObject->getCollisionFlags() | btCollisionObject::CF_NO_CONTACT_RESPONSE);

        dynamicsWorld->addCollisionObject(ghostObject.get());
        ...
        while(RenderWin->IsOpened()) {
			...
			if(input.IsKeyDown(sf::Key::D)) {
                ...
                for(int i = 0; i < ghostObject->getNumOverlappingObjects(); i++) {
                    ...
                    if(pRigidBody) {
                        std::cout << "OMG " << RenderWin->GetFrameTime() << std::endl;
